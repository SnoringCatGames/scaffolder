...
func _update_music_beat_state() -> void:
    ...
    var next_scaled_speed: float = Gs.audio.get_scaled_speed()
    if _last_music_scaled_speed != next_scaled_speed:
        _last_music_scaled_speed = next_scaled_speed
        Gs.audio.set_playback_speed(Gs.audio.playback_speed_multiplier)
    
    var beat_duration_unscaled := get_beat_duration_unscaled()
    
    music_playback_position = Gs.audio.get_playback_position()
...
func get_bpm_unscaled() -> float:
    return Gs.audio._inflated_music_config[ \
                    Gs.audio._current_music_name].bpm if \
            Gs.audio.get_is_music_playing() else \
            INF
...
func get_meter() -> int:
    return Gs.audio._inflated_music_config[ \
                    Gs.audio._current_music_name].meter if \
            Gs.audio.get_is_music_playing() else \
            -1
...
func calculate_path_beat_hashes_for_current_mode(
    ...
    if Gs.slow_motion.get_is_enabled_or_transitioning():
        return calculate_path_beat_hashes(
                path,
                elapsed_path_time,
                Gs.slow_motion.music.time_to_next_music_beat,
                Gs.slow_motion.music.next_music_beat_index,
                Gs.slow_motion.music.music_beat_duration_unscaled,
                Gs.slow_motion.music.meter)
    else:
        return calculate_path_beat_hashes(
                path,
                elapsed_path_time,
