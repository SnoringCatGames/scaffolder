...
func on_value_changed(control_value: float) -> void:
    ...
    if Sc.geometry.are_floats_equal_with_epsilon(
            Sc.camera_controller._manual_zoom, result_value, 0.0001):
        return
    Sc.camera_controller._set_manual_zoom(result_value)
...
func get_value() -> float:
    return _result_value_to_control_value(Sc.camera_controller._manual_zoom)
...
func _on_camera_zoomed() -> void:
    var new_control_value := \
            _result_value_to_control_value(Sc.camera_controller._manual_zoom)
