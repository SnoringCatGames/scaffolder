...
func _ready() -> void:
    assert(Gs.hud_manifest.has("hud_key_value_box_size"))
    assert(Gs.hud_manifest.has("hud_key_value_list_item_manifest"))
    assert(Gs.hud_manifest.has("hud_key_value_box_nine_patch_rect_path"))
    
    var is_hud_key_value_list_shown: bool = \
            !Gs.hud_manifest.hud_key_value_list_item_manifest.empty()
